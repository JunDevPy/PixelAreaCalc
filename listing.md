...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.min_area_spin.setValue(3000)  # Значение по-умолчанию
    ...
    # --- Методы для работы с путями и именами файлов (поддержка кирилицы) --- #
    @staticmethod
    def normalize_path(path):
        """Нормализация пути для корректной работы с русскими символами"""
        return os.path.normpath(os.path.abspath(path))

    @staticmethod
    def check_path_encoding(path):
        """Проверка корректности кодировки пути"""
        try:
            # Попытка открыть директорию/файл
            ...
            return False
    @staticmethod
    def safe_imread(image_path, flags=cv2.IMREAD_GRAYSCALE):
        """Безопасное чтение изображения с поддержкой русских путей"""
        try:
    ...
    @staticmethod
    def safe_img_write(filename, img):
        """Безопасная запись изображения с поддержкой русских путей"""
        try:
    ...
    def process_input_folder(self):
        ...
        self.input_folder = self.normalize_path(self.input_folder_edit.text())
        self.output_folder = self.normalize_path(self.output_folder_edit.text())

        if not self.check_path_encoding(self.input_folder):
    ...
    #     img = self.safe_imread(image_path, cv2.IMREAD_GRAYSCALE)
    #     if img is None:
    #         return [], None
    #
    #     h, w = img.shape
    ...
    #         # self.safe_img_write(output_path, result_img)
    #         self.safe_img_write(result_path, result_img)
    ...
    def run_single_analysis(self, image_path):
        ...
        try:
            ...
            # ИСПРАВЛЕНО: Единое сохранение результата
            result_filename = f"processed_{os.path.basename(image_path)}"
            result_path = os.path.join(self.output_folder, result_filename)

            # Сохраняем результат один раз
            if self.safe_img_write(result_path, result_img):
